...
fn content_mask_alpha(content_mask: ContentMask, position: vec2<f32>) -> f32 {
    let unrounded_mask = content_mask.corner_radii.top_left == 0.0 &&
...
fn fs_quad(input: QuadVarying) -> @location(0) vec4<f32> {
    ...
    let mask_alpha = content_mask_alpha(quad.content_mask, input.position.xy);
...
fn fs_shadow(input: ShadowVarying) -> @location(0) vec4<f32> {
    ...
    return blend_color(input.color, alpha * content_mask_alpha(shadow.content_mask, input.position.xy));
}
...
fn fs_underline(input: UnderlineVarying) -> @location(0) vec4<f32> {
    ...
    let mask_alpha = content_mask_alpha(underline.content_mask, input.position.xy);
...
fn fs_mono_sprite(input: MonoSpriteVarying) -> @location(0) vec4<f32> {
    ...
    return blend_color(input.color, alpha_corrected * content_mask_alpha(sprite.content_mask, input.position.xy));
}
...
fn fs_surface(input: SurfaceVarying) -> @location(0) vec4<f32> {
    ...
    return blend_color(
        ...
        content_mask_alpha(surface_locals.content_mask, input.position.xy)
    );
}
